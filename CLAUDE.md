...
**v1.0 MVP 全部完成**（M1–M5：鉴权 / 空间/位置/分类/标签/成员 CRUD / 物品 CRUD / 仪表盘 / 二维码 / 导入导出 / Docker 部署）。**v1.1.0 已发**（M7 多用户/多空间协作）。**v1.1.1 已发**（Docker 镜像瘦身，650MB → 510MB）。下一步：v1.2 计划（F13 借出归还 / F14 保质期增强收口 / F16 PWA / F18 i18n）。
