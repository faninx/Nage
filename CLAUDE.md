...
仓库根：`data/nage.db`（SQLite，gitignore）+ `data/uploads/`（用户上传图片，v1.4.0 M10 修复后从 `public/uploads/` 迁出）/ `drizzle/0000_*.sql` 起（迁移；开发 `db:push` 直推，部署 `db:generate` + 跑迁移）/ `scripts/`（E2E + 备份恢复 + 调试）。
...
- **better-sqlite3 WAL 模式下 DB 文件被锁**：dev server 跑时手动 `rm data/nage.db*` 会报 "Device or resource busy"。**完整重置协议**：① 停 dev server → ② `rm -f data/nage.db data/nage.db-shm data/nage.db-wal` → ③ 重启（首次启动会自动重建管理员，会清空所有数据，包括上传的图片在 `data/uploads/` 的引用也要清掉）
...
- **`@ducanh2912/next-pwa@10.x` 走 webpack 插件**（M11+ / F16）：Next 16 默认 Turbopack 跟 webpack 冲突。**必须用 `next build --webpack` flag**。Docker build / pnpm build 都用此 flag（package.json scripts 已配）。`manifest.json` 不自动生成 — 必须手写放 `public/manifest.json`。
