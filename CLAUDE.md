...
> **新会话阅读顺序**：
> 1. `C:\Users\Faninx\.claude\projects\...\memory\MEMORY.md`（会话状态 / 未发版 / 未决事项）
> 2. 本文件 `CLAUDE.md`（项目规则）
> 3. `AGENTS.md`（Next.js 16 反直觉点）
> 4. `PRD.md`（功能 / 数据模型 / 技术栈，事实源）

> **写 Next.js 代码前**：必读 `AGENTS.md`，必要时查 `node_modules/next/dist/docs/` 对应章节。本项目用 Next.js 16：`cookies()` 异步、`proxy.ts`（不是 `middleware.ts`）、`params` 是 Promise、`"use server"` 文件只能 `export` async 等都跟训练数据不一样。
...
- `src/lib/db/` — Drizzle schema + 复杂查询。`import "server-only"`，E2E 脚本**不能**直接 import（统一走 HTTP 手动签 JWT + better-sqlite3 直写 DB，见下方"已知坑"）。
